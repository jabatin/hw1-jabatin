...
%%I noticed you didn't specify that codons are actually a set of 3, which I
%%didn't account for in my calculation.
...
while y<(x+1)
    ...
    while a<(b+1)
    Sub = SeqSt(y)- SeqEd(a);
    if rem(Sub, 3) == 0 % New code to account for codons.
        SeqNew = [SeqNew, Sub];
    else
        SeqNew = [SeqNew, -1];
    end
    % SeqNew = [SeqNew, Sub]; Original code deleted to account for codons.
    a = a + 1;
    end
...
%Revised answer after accounting for codons is 21.4%
%%
